...
export class FlyingPlayer extends Player {
    ...
    update(dt) {
        ...
        if (Input.isKeyDown('KeyS') || Input.isKeyDown('ArrowDown'))
            dir.y -= 1;
        if (Math.abs(this.getVel().x) >= FLY_SPEED && (dir.x > 0) === (this.getVel().x > 0))
            dir.x = 0;
        if (Math.abs(this.getVel().y) >= FLY_SPEED && (dir.y > 0) === (this.getVel().y > 0))
            dir.y = 0;
        this.accel(Vector.mul(dir, FLY_ACCEL));
        if (dir.x !== 0)
            this.setFaceDirection(dir.x);
        this.fly();
        super.update(dt);
    }
