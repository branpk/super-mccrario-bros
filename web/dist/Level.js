...
class EntitySeed {
    ...
    instantiate() { return new this.type(this.pos.clone()); }
    instantiateAsTile() { return new this.type(); }
}
export class Level {
    constructor(name, checkpoint) {
        this.running = true;
        this.won = false;
        this.entities = [];
        this.tiles = [];
    ...
    static load(name, checkpoint) {
        return __awaiter(this, void 0, void 0, function* () {
            const lvl = new Level(name, checkpoint);
            yield lvl.loadLevel(name);
            return lvl;
        });
    }
    loadLevel(level) {
        return __awaiter(this, void 0, void 0, function* () {
            const response = yield fetch(`resources/levels/${level}.lvl`);
            const buffer = yield response.arrayBuffer();
            const view = new DataView(buffer);
            let offset = 0;
            const rows = view.getInt32(offset);
            offset += 4;
            const cols = view.getInt32(offset);
            offset += 4;
            this.tiles = Array.from({ length: rows }, () => new Array(cols));
            ...
            while (offset < view.byteLength) {
                ...
                offset += 4;
                this.entitySeeds.push(new EntitySeed(entClass, new Vector(x, y)));
            }
            this.entities = [];
            ...
            let check = 0;
            for (const seed of this.entitySeeds) {
                if (check >= this.checkpoint)
                    break;
                if (seed.type === Checkpoint) {
                    point = seed.pos.clone();
                    check++;
                }
            }
            ...
            for (let i = 0; i < this.toSpawn.length && k < check; ++i) {
                if (this.toSpawn[i].type === Checkpoint) {
    ...
    update(dt) {
        ...
        for (const ent of this.entities) {
            ...
            if (downY < 0)
                downY = 0;
            if (upY >= this.tiles.length)
                upY = this.tiles.length - 1;
            if (ent.getVel().x < 0 && leftX < this.tiles[0].length) {
                let isAir = true;
                for (let r = 0; r <= upY && r < this.tiles.length; ++r) {
                    if (ent.collidesWithTile(this.tiles[r][leftX])) {
                        isAir = false;
                        break;
                    }
                }
                ...
                let isAir = true;
                for (let r = 0; r <= upY && r < this.tiles.length; ++r) {
                    if (ent.collidesWithTile(this.tiles[r][rightX])) {
                        isAir = false;
                        break;
                    }
                }
                if (isAir)
                    ent.lethalCliff(Direction.RIGHT, rightX - ent.getHitSize().w - ent.getPos().x);
            }
            let toCheck = false;
            for (const entB of this.entities) {
                if (toCheck && !entB.isDying())
                    Physics.checkEntities(ent, entB);
            ...
            for (let r = downY; r <= upY; ++r) {
                for (let c = leftX; c <= rightX; ++c) {
                    ...
                    if (ent instanceof Player) {
                        ...
                        else if (tile instanceof BossStop) {
                            this.won = true;
                        }
                    }
                }
        ...
        if (ru.x >= this.tiles[0].length)
            this.camera.x -= ru.x - this.tiles[0].length;
        if (ru.y >= this.tiles.length)
            this.camera.y -= ru.y - this.tiles.length;
        ...
        for (const row of this.tiles)
            for (const tile of row)
                if (tile instanceof BossBrick)
                    tile.setExists(bossExists);
        if (player)
            for (const ent of this.entities)
                if (ent instanceof BossBaal)
                    ent.setPlayerPos(player.getPos());
    }
    switchDebugMode() {
        for (let i = 0; i < this.entities.length; ++i) {
            const ent = this.entities[i];
    ...
    render(ctx) {
        ...
        for (const ent of this.entities) {
            ...
            const entRU = Vector.add(ent.getPos(), ent.getImageSize().toVector());
            if (entRU.x <= ld.x || entLD.x >= ru.x || entRU.y <= ld.y || entLD.y >= ru.y)
                continue;
            ctx.save();
            ent.render(ctx);
            if (DebugInfo.debug)
                ent.renderHitBox(ctx);
            ctx.restore();
        }
        ctx.restore();
    }
