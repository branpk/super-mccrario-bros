...
export class Player extends Entity {
    ...
    update(dt) {
        ...
        if (Input.isKeyDown('KeyD') || Input.isKeyDown('ArrowRight'))
            dir += 1;
        if (Math.abs(this.getVel().x) > WALK_SPEED && (dir > 0) === (this.getVel().x > 0))
            dir = 0;
        ...
        if (jumpKey && this.bouncing)
            this.jumping = true;
        if (this.bouncing && this.bounceTime > BOUNCE_TIME)
            this.bouncing = false;
        if (this.jumping && (!jumpKey || this.jumpTime > JUMP_TIME))
            this.jumping = false;
